import { CARD_SUITS, CARD_RANKS, CARD_VALUES, SHUFFLE_RANDOMIZATION_FACTOR } from "./utils/constants.js";
...
export default class Deck {
  ...
  resetDeck(shuffle = true) {
    this.deck = []; //respresents the deck, bottom = first, top = last
  ...
  shuffleDeck() {
    ...
    if (this.getRemainingCardsCount() > 0) {
      for (let i = 0; i < SHUFFLE_RANDOMIZATION_FACTOR; i++) {
